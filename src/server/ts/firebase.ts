...
export class Firebase {
    ...
    timeoutAutomations: Array<{
        automationID: string,
        timeout: any
    }>;
    resetAutomationTimeoutIfActive(automation: DatabaseData) {
    ...
    _checkSensorAutomation(updatePath: string){   
        let oldVal = this.readFromLocalDB(updatePath, 0);
        let newVal = this._sensorsUpdates[updatePath];     
        Object.values(this.readFromLocalDB("automations", {})).forEach((automation: DatabaseData) => {
...
*/
     ...
     */
    private readFromDBObject(dbObject: any, path: string, defaultValueIfPathNotExists?: any): any {
     ...
     */
    public async writeToLocalDB(path: string, val: any, time: string | number) {
        if (time == undefined) {
            time = Date.now();
        }
     ...
     * @param time Čas, který se má zapsat jako poslední změny
     */
    public async removeInLocalDB(path: string, time: string | number) {
        if (time == undefined) {
            time = Date.now();
        }
     ...
     */
    public async clientUpdateInDB(bodyData: IFormData) {
     ...
     */
    public async clientPushToDB(bodyData: IFormData) {
        let path = this.correctPath(bodyData.path);
     ...
     * @param bodyData Data z těla POST požadavku, obsahují cestu v databázi, na kterou se klient "ptá".
     */
    public async clientRemoveFromDB(bodyData: IFormData) {
        let path = this.correctPath(bodyData.path);
