...
export class ErrorLogger {
    ...
    private _fd;
    private initialWritingToFilePromiseResolver;
    private _writingToFilePromise = new Promise((resolve, reject) => { this.initialWritingToFilePromiseResolver = resolve; })
    private _logCount = 0;

    static ERROR_LOG_FILE_PATH = "log.txt";
    static ERROR_HTML_LOG_FILE_PATH = "log.html";
    constructor() {
    ...
    private async logToFile(nativeError: any, errorInfo: { errorDescription: string, placeID: number, reaction?: string }, state: object, exitCode: number = -1) {
        ...
        let content =
            `Čas vzniku chyby: ${new Date().toLocaleString()}${lineBreak}` +
        ...
        let contentHTML =
            `<pre style="background-color: #def0ff">` +
            `<code>` +
            `<hr>` +
    ...
    private async _log(nativeError: any, errorInfo: { errorDescription: string, placeID: number, reaction?: string }, state: object, exitCode: number = -1) {
        ...
        if (ConfigReader.getValue("debugLevel", 0) > 0 || exitCode > -1) { // Pokud je debugLevel > 0, resp. chyba je fatální, zobrazit info o chybě
            let id = "";
        ...
        if (exitCode > -1) {
            ...
            let exitTimeout = 0;
            if (ConfigReader.getValue("showLogFilesOnError", false)) {
