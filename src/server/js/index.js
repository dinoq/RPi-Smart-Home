...
class ServerApp {
    constructor() {
        ...
        this._app.post('/*', (req, res) => {
            ...
            res.redirect("http://localhost/domu");
        ...
        this._firebase.login(config_js_1.config.username, config_js_1.config.password);
    ...
    start(port) {
        var server = this._app.listen(port || config_js_1.config.port);
    }
}
new ServerApp().start();
