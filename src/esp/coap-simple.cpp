...
bool Coap::startMulticast(IPAddress localIP, IPAddress multicastIP) {
    this->startMulticast(localIP, multicastIP, COAP_DEFAULT_PORT);
    return true;
}

bool Coap::startMulticast(IPAddress localIP, IPAddress multicastIP, int port) {
    this->_udp->beginMulticast(WiFi.localIP(), multicastIP, port);
    return true;
}
